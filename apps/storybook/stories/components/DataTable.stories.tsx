...
import { Download, Filter, Plus, Trash2, Edit, Eye, MoreVertical, ChevronDown, ChevronRight } from 'lucide-react';
...
const generateUsers = (count: number): User[] => {
  ...
  const skills = ['React', 'TypeScript', 'Node.js', 'Python', 'AWS', 'Docker', 'GraphQL', 'MongoDB'];
  const statuses = [
  ...
  return Array.from({ length: count }, (_, i) => ({
    ...
    avatar: {
      ...
      imageUrl: i % 3 === 0 ? `https://i.pravatar.cc/150?img=${i + 1}` : undefined,
      initials: `U${i + 1}`
    } as AvatarData,
    joinDate: new Date(2020 + Math.floor(Math.random() * 4), Math.floor(Math.random() * 12), Math.floor(Math.random() * 28)),
    salary: 50000 + Math.floor(Math.random() * 100000),
    skills: skills.sort(() => 0.5 - Math.random()).slice(0, Math.floor(Math.random() * 4) + 1)
  }));
};
...
const ServerSideDataTable: React.FC = () => {
  ...
  const fetchData = async (page: number, pageSize: number, sort?: SortConfig, search?: SearchConfig) => {
    setLoading(true);
  ...
  return (
    <DataTable
      ...
      bulkActions={
        selectedRows.length > 0 && (
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <span style={{ marginRight: '16px' }}>{selectedRows.length} selected</span>
            <Button
              buttonType={ButtonType.SECONDARY}
              size={ButtonSize.SMALL}
              text="Assign"
        ...
        return (
          <div style={{ padding: '24px', backgroundColor: '#f9fafb' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
              <div>
                <h5 style={{ marginBottom: '8px', color: '#6b7280' }}>Contact Information</h5>
                <p><strong>Email:</strong> {user.email}</p>
                <p><strong>Phone:</strong> +1 (555) 123-4567</p>
                <p><strong>Address:</strong> 123 Main St, City, State 12345</p>
              </div>
              <div>
                <h5 style={{ marginBottom: '8px', color: '#6b7280' }}>Employment Details</h5>
              ...
              <div>
                <h5 style={{ marginBottom: '8px', color: '#6b7280' }}>Skills & Status</h5>
                <p><strong>Status:</strong> <span style={{ 
                  padding: '2px 8px', 
                  borderRadius: '4px',
                  backgroundColor: user.status.color === 'success' ? '#d1fae5' : 
                                  user.status.color === 'error' ? '#fee2e2' :
                                  user.status.color === 'warning' ? '#fef3c7' : '#f3f4f6',
                  color: user.status.color === 'success' ? '#065f46' : 
                         user.status.color === 'error' ? '#991b1b' :
                         user.status.color === 'warning' ? '#92400e' : '#374151'
                }}>{user.status.text}</span></p>
