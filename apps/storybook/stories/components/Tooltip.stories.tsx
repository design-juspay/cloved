...
import { Tooltip, TooltipSide, TooltipAlign, TooltipSize, TooltipSlotDirection, Button, ButtonType } from 'blend-v1';
import { Info, HelpCircle, Star, AlertTriangle, CheckCircle, Settings, Zap, Shield, Heart, Trophy } from 'lucide-react';
...
\`\`\`
  ...
  argTypes: {
    ...
    side: {
      ...
      description: 'The side where the tooltip should appear relative to the trigger',
    },
    align: {
      control: 'select',
    ...
    children: {
      description: 'The trigger element that will show the tooltip on hover/focus',
    },
  },
  tags: ['autodocs'],
};
...
export const TooltipPositions: Story = {
  render: () => (
    <div style={{ 
      ...
      <Tooltip content="Center tooltip - no arrow" side={TooltipSide.TOP} showArrow={false}>
        <Button buttonType={ButtonType.PRIMARY} text="Center" />
      </Tooltip>
      <Tooltip content="Right tooltip" side={TooltipSide.RIGHT} showArrow={true}>
        <Button buttonType={ButtonType.SECONDARY} text="Right" />
      </Tooltip>
      
      <div></div>
      <Tooltip content="Bottom tooltip" side={TooltipSide.BOTTOM} showArrow={true}>
  ...
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '40px', padding: '40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Tooltip content="Start aligned tooltip" side={TooltipSide.TOP} align={TooltipAlign.START} showArrow={true}>
          <Button buttonType={ButtonType.SECONDARY} text="Start Align" />
        </Tooltip>
        <Tooltip content="Center aligned tooltip" side={TooltipSide.TOP} align={TooltipAlign.CENTER} showArrow={true}>
          <Button buttonType={ButtonType.PRIMARY} text="Center Align" />
        </Tooltip>
        <Tooltip content="End aligned tooltip" side={TooltipSide.TOP} align={TooltipAlign.END} showArrow={true}>
          <Button buttonType={ButtonType.SECONDARY} text="End Align" />
        </Tooltip>
      </div>
      
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Tooltip content="Start aligned (right)" side={TooltipSide.RIGHT} align={TooltipAlign.START} showArrow={true}>
          <Button buttonType={ButtonType.SECONDARY} text="Right Start" />
        </Tooltip>
        <Tooltip content="Center aligned (right)" side={TooltipSide.RIGHT} align={TooltipAlign.CENTER} showArrow={true}>
          <Button buttonType={ButtonType.PRIMARY} text="Right Center" />
        </Tooltip>
        <Tooltip content="End aligned (right)" side={TooltipSide.RIGHT} align={TooltipAlign.END} showArrow={true}>
  ...
  render: () => (
    <div style={{ display: 'flex', gap: '24px', alignItems: 'center', flexWrap: 'wrap' }}>
      <Tooltip 
        ...
        showArrow={true}
      >
        <Button buttonType={ButtonType.PRIMARY} text="Rich Content" leadingIcon={Info} />
      </Tooltip>
      ...
      >
        <Button buttonType={ButtonType.SECONDARY} text="Status" leadingIcon={CheckCircle} />
      </Tooltip>
      
      <Tooltip 
        content={
          <div>
            <div style={{ color: '#fbbf24', fontWeight: 'bold', marginBottom: '4px' }}>⚠️ Warning</div>
      ...
      >
        <Button buttonType={ButtonType.SECONDARY} text="Warning" leadingIcon={AlertTriangle} />
      </Tooltip>
    </div>
  ),
  parameters: {
  ...
  render: () => (
    <div style={{ display: 'flex', gap: '24px', alignItems: 'center', flexWrap: 'wrap' }}>
      ...
      <Tooltip 
        ...
        side={TooltipSide.BOTTOM}
      >
        <Button buttonType={ButtonType.SECONDARY} text="Security" leadingIcon={Settings} />
      </Tooltip>
    </div>
  ),
  parameters: {
...
export const PracticalExamples: Story = {
  render: () => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', padding: '20px' }}>
      <Tooltip 
        content="Add this item to your favorites list"
        size={TooltipSize.LARGE}
        showArrow={true}
      >
        <Button buttonType={ButtonType.SECONDARY} text="Add to Favorites" leadingIcon={Heart} />
      </Tooltip>
      ...
      <Tooltip 
        ...
        slot={<Zap size={16} color="#f59e0b" />}
        slotDirection={TooltipSlotDirection.LEFT}
        size={TooltipSize.LARGE}
        showArrow={true}
        side={TooltipSide.RIGHT}
      >
        <Button buttonType={ButtonType.PRIMARY} text="Optimize" leadingIcon={Zap} />
      </Tooltip>
      
      <Tooltip 
        content="View your achievements and progress"
        size={TooltipSize.LARGE}
        showArrow={true}
        side={TooltipSide.BOTTOM}
      >
        <Button buttonType={ButtonType.SECONDARY} text="Achievements" leadingIcon={Trophy} />
      </Tooltip>
      
      <Tooltip 
        content={
          <div>
            <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>Advanced Settings</div>
            <div>Configure advanced options and preferences</div>
          </div>
        }
      ...
      >
        <Button buttonType={ButtonType.SECONDARY} text="Settings" leadingIcon={Settings} />
      </Tooltip>
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story: 'Real-world tooltip examples for common UI patterns: favorites, optimization, achievements, and settings.',
      },
    },
  },
};
...
export const Playground: Story = {
  ...
  render: (args) => (
    <Tooltip {...args}>
      <Button buttonType={ButtonType.PRIMARY} text="Hover me" />
    </Tooltip>
  ),
  parameters: {
    docs: {
      description: {
        story: 'Interactive playground to test different tooltip configurations. Use the controls panel below to experiment with different props.',
      },
    },
  },
};
  ...
  render: () => (
    <div style={{ display: 'flex', gap: '20px', alignItems: 'center', flexDirection: 'column' }}>
      <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
        <Tooltip 
          content="This button submits the form (accessible via keyboard)"
          showArrow={true}
          size={TooltipSize.LARGE}
        >
          <Button buttonType={ButtonType.PRIMARY} text="Submit Form" />
        </Tooltip>
        
        <Tooltip 
          content="Navigate using Tab key to focus and see this tooltip"
          showArrow={true}
          size={TooltipSize.LARGE}
          side={TooltipSide.RIGHT}
        >
          <Button buttonType={ButtonType.SECONDARY} text="Info" leadingIcon={Info} />
        </Tooltip>
      </div>
      
      <div style={{ fontSize: '14px', color: '#666', textAlign: 'center', maxWidth: '400px' }}>
        <strong>Accessibility Note:</strong> These tooltips are keyboard accessible. 
        Use Tab to navigate and focus elements to see their tooltips.
      </div>
    </div>
  ),
  parameters: {
    docs: {
      description: {
        story: 'Tooltip accessibility example showing keyboard navigation support. Use Tab key to navigate and see tooltips appear on focus.',
      },
    },
  },
}; 