...
export type TableHeaderProps<T extends Record<string, unknown>> = {
  ...
  onColumnFilter?: (field: string, type: FilterType, value: string | string[] | { min: number; max: number }, operator?: 'equals' | 'contains' | 'startsWith' | 'endsWith' | 'gt' | 'lt' | 'gte' | 'lte' | 'range') => void;
