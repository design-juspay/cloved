...
export const createSortHandlers = (
  ...
  const handleSortAscending = (field: string) => {
    ...
    if (sortState.currentSortField !== field || sortState.currentSortDirection !== SortDirection.ASCENDING) {
      setSortState({
        currentSortField: field,
        currentSortDirection: SortDirection.ASCENDING
      });
      
      if (sortState.currentSortField === field && sortState.currentSortDirection === SortDirection.DESCENDING) {
        onSort(fieldKey);
...
export const createFilterHandlers = (
  ...
  const handleSearchChange = (fieldKey: string, value: string) => {
    setFilterState(prev => ({
      ...prev,
      columnSearchValues: { ...prev.columnSearchValues, [fieldKey]: value }
    }));
  };
  ...
  const handleMultiSelectFilter = (column: ColumnDefinition<Record<string, unknown>>, fieldKey: string, value: string, onColumnFilter?: ColumnFilterHandler) => {
    setFilterState(prev => {
      ...
      return {
        ...prev,
        columnSelectedValues: { ...prev.columnSelectedValues, [fieldKey]: newSelected }
      };
    });
  };
