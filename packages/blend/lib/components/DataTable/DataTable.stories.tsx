...
\`\`\`
  ...
  argTypes: {
    ...
    serverSidePagination: {
      description: 'Handle pagination on server-side (disables local pagination)',
      control: 'boolean',
    },
    isLoading: {
...
const createUser = (
  ...
  status: {
    text: status,
    color: status === 'Active' ? 'success' : status === 'Inactive' ? 'error' : status === 'Pending' ? 'warning' : 'neutral',
    variant: 'subtle',
  } as TagData,
  salary,
  joinDate,
...
const sampleUsers: User[] = [
  createUser(1, 'John Doe', 'john@example.com', 'Admin', 'Engineering', 'Active', 120000, '2020-01-15', '2 hours ago'),
  createUser(2, 'Jane Smith', 'jane@example.com', 'Manager', 'Marketing', 'Active', 95000, '2019-03-10', '1 day ago'),
  createUser(3, 'Bob Johnson', 'bob@example.com', 'Developer', 'Engineering', 'Inactive', 85000, '2021-07-22', '1 week ago'),
  createUser(4, 'Alice Brown', 'alice@example.com', 'Designer', 'Design', 'Active', 75000, '2020-11-05', '3 hours ago'),
  createUser(5, 'Charlie Wilson', 'charlie@example.com', 'Analyst', 'Finance', 'Pending', 70000, '2022-02-14', '2 days ago'),
  createUser(6, 'Diana Lee', 'diana@example.com', 'Developer', 'Engineering', 'Active', 90000, '2021-09-18', '5 hours ago'),
  createUser(7, 'Eve Davis', 'eve@example.com', 'Manager', 'Sales', 'Active', 100000, '2019-12-03', '1 hour ago'),
  createUser(8, 'Frank Miller', 'frank@example.com', 'Support', 'Customer Service', 'Suspended', 55000, '2022-04-20', '3 weeks ago'),
];
      ...
      return (
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#6b7280' }}>{avatarData.sublabel}</div>
          </div>
        </div>
      );
    },
  ...
  },
  {
    ...
    renderCell: (value: unknown) => {
      ...
      const getStatusColor = (status: string): TagColor => {
        switch (status) {
          case 'Active': return TagColor.SUCCESS;
          case 'Inactive': return TagColor.ERROR;
          case 'Pending': return TagColor.WARNING;
          case 'Suspended': return TagColor.NEUTRAL;
